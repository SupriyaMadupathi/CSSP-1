...
# You'll notice that in Problem 2, your monthly payment had to be a
 multiple of $10. Why did we make it that way? You can try running
...
# Write a program that uses these bounds and bisection search (for more info
    check out the Wikipedia page on bisection search) to find
# the smallest monthly payment to the cent (no more multiples of $10) such that
 we can pay off the debt within a year. Try it out with
# large inputs, and notice how fast it is (try the same large inputs in your
    solution to Problem 2 to compare!). Produce the same return
# value as you did in Assignment 2.
@ author SupriyaMadupathi
'''


def payingdebtoffinayear(b_i, annual_int):
    '''
    Bisection searching
    '''
    init_bal = b_i
    monint_ra = annual_int/12
    low_i = init_bal/12
    up_i = (init_bal* (1+ monint_ra)**12)/12.0
    epsilon = 0.03
    while abs(b_i) > epsilon:
        mon_payrate = (up_i + low_i)/2
        b_i = init_bal
        for _ in range(12):
            ans_i = b_i - mon_payrate
            b_i = ans_i + (ans_i * monint_ra)
        if b_i > epsilon:
            low_i = mon_payrate
        elif b_i < -epsilon:
            up_i = mon_payrate
